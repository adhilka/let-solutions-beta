    ...
    return (
      <div className="max-w-7xl mx-auto px-4 py-20 bg-[var(--color-surface)]">
        <div className="text-center max-w-2xl mx-auto mb-16">
          <h1 className="text-4xl font-extrabold mb-4 text-white">Course Not Found</h1>
          <p className="text-[var(--color-text-secondary)] mb-8 text-lg">The course you are looking for does not exist or has been moved. Check out our other popular programs below.</p>
          ...
          {FAILSAFE_COURSES.slice(0, 3).map((c) => (
            <Link key={c.id} to={`/courses/${c.slug}`} className="group bg-[var(--color-surface-alt)] border border-[var(--color-border)] rounded-3xl overflow-hidden shadow-sm hover:shadow-md transition-all">
              <div className="aspect-video relative overflow-hidden">
                <img src={c.imageUrl} alt={c.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500 opacity-80 group-hover:opacity-100" />
              </div>
              <div className="p-6">
                <h3 className="font-bold text-lg mb-2 group-hover:text-[var(--color-primary-400)] transition-colors text-white">{c.title}</h3>
                <p className="text-[var(--color-text-secondary)] text-sm line-clamp-2">{c.shortDescription}</p>
              </div>
            </Link>
          ))}
        </div>
      ...
      <div className="bg-[#0B1120] border-b border-slate-800 pt-6 pb-40">
        <div className="max-w-7xl mx-auto px-4">
          <motion.div 
            ...
            className="flex items-center gap-2 text-xs md:text-sm text-slate-400 mb-12"
          >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            ...
            <motion.div 
              ...
              className="hidden lg:block relative z-10 w-full rounded-[2rem] overflow-hidden shadow-2xl shadow-blue-500/10 border border-white/10 aspect-video transform"
            >
              <img 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 md:gap-12">
          ...
          <div className="lg:col-span-2 space-y-10">
            ...
            <motion.div 
              ...
              viewport={{ once: true }}
              className="lg:hidden rounded-[2rem] overflow-hidden shadow-2xl border-4 border-[#0B1120] aspect-video"
            >
              <img 
                src={course.imageUrl || 'https://images.unsplash.com/photo-1597872200969-2b65d56bd16b?ixlib=rb-4.0.3&auto=format&fit=crop&w=1200&q=80'} 
                alt={course.title} 
            ...
            >
              ...
              <h2 className="text-2xl md:text-3xl font-extrabold text-white mb-6 tracking-tight">About the Course</h2>
              <div className="prose prose-invert prose-lg max-w-none text-[var(--color-text-secondary)] leading-relaxed whitespace-pre-wrap relative z-10 font-medium font-body">
            ...
            <div className="w-full">
              <h3 className="text-2xl font-extrabold text-white mb-6 px-2 tracking-tight">Key Highlights</h3>
              <motion.div 
              ...
              >
                {highlights.map((item: string, i: number) => (
                  <motion.div 
                    ...
                    className="flex items-start gap-4 bg-[var(--color-surface-alt)] p-5 rounded-2xl border border-[var(--color-border)] shadow-sm hover:border-[var(--color-primary-600)] transition-all group"
                  >
                    <div className="bg-[var(--color-primary-900)] text-[var(--color-primary-400)] p-2 rounded-xl group-hover:scale-110 group-hover:bg-[var(--color-primary-600)] group-hover:text-white transition-all">
                      <CheckCircle size={20} strokeWidth={2.5} />
                    </div>
                    <span className="font-semibold text-white leading-snug">{item}</span>
                  </motion.div>
                ))}
              </motion.div>
            </div>
          ...
          >
            <div className="sticky top-24 space-y-6">
              <div className="bg-[var(--color-surface-alt)] rounded-[2.5rem] border border-[var(--color-border)] shadow-xl p-8 relative overflow-hidden">
                {/* Decorative blob */}
                <div className="absolute -top-10 -right-10 w-32 h-32 bg-[var(--color-primary-900)] rounded-full blur-2xl opacity-20"></div>
                
                {pinnedOffer && (
                  <div className="bg-gradient-to-br from-[var(--color-primary-600)] to-indigo-900 rounded-2xl p-6 mb-8 relative overflow-hidden text-white shadow-lg shadow-black/40 group transform transition-all hover:scale-[1.02]">
                    <div className="absolute right-0 top-0 opacity-10 group-hover:opacity-20 transition-opacity text-white">
                      <Award size={100} className="transform translate-x-1/4 -translate-y-1/4" />
                    </div>
                    <div className="relative z-10">
                      <span className="text-[10px] font-extrabold uppercase tracking-widest text-white/70 mb-2 block">{pinnedOffer.badgeLabel || 'Exclusive Offer'}</span>
                      <h4 className="font-extrabold text-white text-lg mb-2">{pinnedOffer.headline}</h4>
                      <p className="text-sm text-white/80 mb-4 leading-relaxed">{pinnedOffer.subtext}</p>
                    </div>
                  </div>
                )}
                ...
                <div className="mb-8">
                  {pinnedOffer?.discountedFee ? (
                    <div className="flex flex-col">
                      ...
                      <span className="text-4xl md:text-5xl font-extrabold text-[var(--color-neon-green)] tracking-tight">
                    ...
                    <div className="py-4 border-b border-dashed border-[var(--color-border)] mb-6">
                       <span className="text-lg font-bold text-[var(--color-text-secondary)]">Course Fee: On Enquiry</span>
                    </div>
                  )}
                  {course.badge && <span className="inline-block mt-4 badge badge-red border-[var(--color-error)] text-[var(--color-error)] bg-[var(--color-error)]/10">{course.badge}</span>}
                </div>
                ...
                {course.feeStructure && (course.feeStructure.registrationFee || course.feeStructure.description) && (
                  <div className="space-y-4 mb-8">
                    {course.feeStructure.registrationFee && (
                      <div className="flex justify-between items-center bg-[var(--color-surface)] p-4 rounded-2xl border border-[var(--color-border)]">
                    ...
                    {course.feeStructure.description && (
                      <div className="p-4 bg-[var(--color-primary-900)] rounded-2xl border border-[var(--color-primary-800)]">
                        <p className="text-xs text-[var(--color-primary-200)] font-medium leading-relaxed">
                          {course.feeStructure.description}
                        </p>
                      </div>
                    )}
                ...
                <div className="space-y-4 mb-8 bg-[var(--color-surface)] rounded-2xl p-5 border border-[var(--color-border)]">
                ...
                <Link to="/contact" className="block w-full text-center py-4 px-6 text-base font-bold text-white btn-primary rounded-xl transition-all shadow-lg active:translate-y-0">
                  Enquire for Details
                </Link>

                <p className="text-xs font-semibold text-center text-[var(--color-text-tertiary)] mt-4">
                  Questions? <Link to="/contact" className="text-[var(--color-primary-400)] hover:text-white transition-colors">Contact Support</Link>
                </p>
              </div>
            </div>
          </motion.div>
