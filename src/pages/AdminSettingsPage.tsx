...
export default function AdminSettingsPage() {
  ...
  useEffect(() => {
    async function fetchSettings() {
      try {
        const db = getReadDb();
        const settingsDoc = await getDoc(doc(db, 'artifacts/tech-institute/public/data/settings', 'global'));
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await dualWrite(['artifacts', 'tech-institute', 'public', 'data', 'settings', 'global'], updatedSettings);
      ...
      setModalConfig({
        ...
        message: 'Your branding and configuration settings have been updated successfully.',
  ...
  const clearCollection = async (collectionPath: string) => {
    try {
      const db = getReadDb();
      const querySnapshot = await getDocs(collection(db, `artifacts/tech-institute/public/data/${collectionPath}`));
      const deletePromises = querySnapshot.docs.map(d => 
        dualDelete(['artifacts', 'tech-institute', 'public', 'data', collectionPath, d.id])
      );
  ...
  const handleClearAllData = async () => {
    setIsSaving(true);
    const collections = ['courses', 'testimonials', 'batches', 'enquiries', 'offers', 'posts', 'settings'];
    let successCount = 0;
    ...
    if (successCount === collections.length) {
      setModalConfig({
        ...
        message: 'All dynamic content has been successfully removed from the server. Your site is now in its original clean state.',
        confirmVariant: 'success',
        confirmText: 'Done',
        mode: 'status',
        onConfirm: () => {}
    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-[var(--color-text-secondary)] text-sm">Full-site CMS management.</p>
        </div>
           ...
           <div className="max-w-2xl">
             ...
             {activeTab === 'branding' && (
                <div className="space-y-6">
                  <div>
                    <label className="block text-sm font-medium text-[var(--color-text-secondary)] mb-2">Institute Name</label>
                    <input type="text" className="input" value={settings.branding?.instituteName || ''} onChange={e => updateSetting('branding', 'instituteName', e.target.value)} />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-[var(--color-text-secondary)] mb-2">Tagline</label>
                    <input type="text" className="input" value={settings.branding?.tagline || ''} onChange={e => updateSetting('branding', 'tagline', e.target.value)} />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-[var(--color-text-secondary)] mb-2">Logo</label>
                    <div className="flex items-center gap-4">
                      {settings.branding?.logoUrl && <img src={settings.branding.logoUrl} className="h-10 w-auto rounded border" alt="Logo" />}
                      <input type="file" accept="image/*" onChange={e => setLogoFile(e.target.files?.[0] || null)} className="text-xs" />
                    </div>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-[var(--color-text-secondary)] mb-2">Favicon</label>
                    <div className="flex items-center gap-4">
                      {settings.branding?.faviconUrl && <img src={settings.branding.faviconUrl} className="h-8 w-8 rounded border" alt="Favicon" />}
                      <input type="file" accept="image/*" onChange={e => setFaviconFile(e.target.files?.[0] || null)} className="text-xs" />
                    </div>
                  </div>
                </div>
             )}
             ...
             {activeTab === 'contact' && (
                <div className="space-y-6">
                  <div>
                    <label className="block text-sm font-medium text-[var(--color-text-secondary)] mb-2">Primary Email</label>
                    <input type="email" className="input" value={settings.contact?.email || ''} onChange={e => updateSetting('contact', 'email', e.target.value)} />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-[var(--color-text-secondary)] mb-2">Phone</label>
                    <input type="text" className="input" value={settings.contact?.phone || ''} onChange={e => updateSetting('contact', 'phone', e.target.value)} />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-[var(--color-text-secondary)] mb-2">WhatsApp Number</label>
                    <input type="text" className="input" value={settings.contact?.whatsapp || ''} onChange={e => updateSetting('contact', 'whatsapp', e.target.value)} />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-[var(--color-text-secondary)] mb-2">Address</label>
                    <textarea className="input" rows={3} value={settings.contact?.address || ''} onChange={e => updateSetting('contact', 'address', e.target.value)}></textarea>
                  </div>
                </div>
             )}
             ...
             {activeTab === 'stats' && (
                <div className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <label className="block text-sm font-medium text-[var(--color-text-secondary)] mb-2">Years Excellence</label>
                      <input type="text" className="input" value={settings.stats?.yearsExcellence || ''} onChange={e => updateSetting('stats', 'yearsExcellence', e.target.value)} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-[var(--color-text-secondary)] mb-2">Students Trained</label>
                      <input type="text" className="input" value={settings.stats?.studentsTrained || ''} onChange={e => updateSetting('stats', 'studentsTrained', e.target.value)} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-[var(--color-text-secondary)] mb-2">Placement Rate</label>
                      <input type="text" className="input" value={settings.stats?.placementRate || ''} onChange={e => updateSetting('stats', 'placementRate', e.target.value)} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-[var(--color-text-secondary)] mb-2">Course Modules</label>
                      <input type="text" className="input" value={settings.stats?.courseModules || ''} onChange={e => updateSetting('stats', 'courseModules', e.target.value)} />
                    </div>
                  </div>
                </div>
             )}
             ...
             {activeTab === 'admissions' && (
                <div className="space-y-6">
                  <div>
                    <label className="block text-sm font-medium text-[var(--color-text-secondary)] mb-2">Year Display Mode</label>
                    <select 
                      className="input" 
                      value={settings.admissions?.mode || 'auto'} 
                      onChange={e => updateSetting('admissions', 'mode', e.target.value)}
                    >
                      <option value="auto">Auto (Calculates Current Academic Year)</option>
                      <option value="manual">Manual Override</option>
                    </select>
                    <p className="mt-2 text-xs text-gray-500">Auto mode will show "{new Date().getMonth() > 4 ? `${new Date().getFullYear()}-${(new Date().getFullYear() + 1).toString().slice(-2)}` : `${new Date().getFullYear() - 1}-${new Date().getFullYear().toString().slice(-2)}`}"</p>
                  </div>
                  {settings.admissions?.mode === 'manual' && (
                    <div>
                      <label className="block text-sm font-medium text-[var(--color-text-secondary)] mb-2">Manual Admission Text</label>
                      <input 
                        type="text" 
                        className="input" 
              ...
              {activeTab === 'maintenance' && (
                <div className="space-y-10">
                  <div className="bg-amber-50 border border-amber-200 rounded-3xl p-6 flex flex-col md:flex-row gap-6 items-start">
                    ...
                    <div>
                      <h3 className="text-xl font-bold text-amber-900 mb-2">Danger Zone</h3>
                      <p className="text-amber-800 text-sm leading-relaxed mb-4">
                        The following actions are irreversible. They will permanently remove data from your server. Use with extreme caution.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="p-6 rounded-3xl border border-slate-200 hover:border-red-200 hover:bg-red-50/30 transition-all group">
                      <div className="flex items-center gap-3 mb-4">
                        ...
                        <h4 className="font-bold text-slate-900">Clear Leads & Enquiries</h4>
                      </div>
                      <p className="text-slate-500 text-xs mb-6">Deletes all student enquiries and contact form submissions.</p>
                      <button 
                        onClick={() => setModalConfig({
                          isOpen: true,
                          title: 'Clear Enquiries?',
                          message: 'This will permanently delete all student enquiries and messages. You cannot undo this action.',
                          confirmVariant: 'danger',
                          ...
                          onConfirm: async () => {
                            ...
                            if (res) {
                              setModalConfig({
                                ...
                                message: 'All student leads and enquiry records have been removed.',
                                confirmVariant: 'success',
                                confirmText: 'Done',
                                mode: 'status',
                                onConfirm: () => {}
                    ...
                    <div className="p-6 rounded-3xl border border-slate-200 hover:border-red-200 hover:bg-red-50/30 transition-all group">
                      <div className="flex items-center gap-3 mb-4">
                        ...
                        <h4 className="font-bold text-slate-900">Universal Server Wipe</h4>
                      </div>
                      <p className="text-slate-500 text-xs mb-6">Deletes EVERYTHING: Courses, Blog Posts, Batches, Feedbacks, and your Branding/Stats settings.</p>
                      <button 
                        onClick={() => setModalConfig({
                          isOpen: true,
                          title: 'DELETE EVERYTHING?',
                          message: 'WARNING: This will wipe all dynamic content AND your custom settings (Branding, Stats, etc.) from the server. The site will return to factory defaults.',
                          confirmVariant: 'danger',
