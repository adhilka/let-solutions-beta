...
export default function BlogListPage() {
  ...
  return (
    <>
      <Helmet>
        <title>Tech Blog & Updates | Let Solutions</title>
        <meta name="description" content="Read our latest blog posts, technical guides, and industry news." />
      </Helmet>
      ...
      <div className="max-w-[var(--container-xl)] mx-auto px-4 sm:px-6 lg:px-8 py-16">
         {isLoading ? (
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                 {posts.map((post: any) => (
                     <div key={post.id} className="bg-white border flex flex-col border-[var(--color-border)] rounded-[var(--radius-xl)] shadow-[var(--shadow-card)] overflow-hidden">
                         <div className="relative aspect-video bg-gray-200">
                             {post.coverImage?.url && <img src={post.coverImage.url} alt={post.title} className="w-full h-full object-cover" />}
                         </div>
                         <div className="p-6 flex flex-col flex-1">
                             ...
                             <div className="flex items-center gap-2 pt-4 border-t border-[var(--color-border)] mt-auto">
                                 {post.author?.avatarUrl ? (
                                     ...
                                     <div className="w-8 h-8 rounded-full bg-[var(--color-primary-100)]" />
                                 )}
                                 <div className="text-xs">
                                     <div className="font-semibold">{post.author?.name || 'Admin'}</div>
                                     <div className="text-[var(--color-text-tertiary)]">{new Date(post.publishedAt?.toDate?.() || Date.now()).toLocaleDateString()}</div>
                                 </div>
                             </div>
                         </div>
                     </div>
